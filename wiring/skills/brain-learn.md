...
Spec: `Khaan/brain/docs/skills.html` § Brain Learn. Orchestrator: `engines/brain_learn.py::run(source)`.

## Manual invocation

```bash
python3 -c "import sys; sys.path.insert(0, '/home/ubuntu/.claude/brain'); from engines.brain_learn import run; import json; r = run('$ARGUMENTS'); print(json.dumps(r, indent=2) if r else 'learn returned None')"
```
