...
def phase_ingest():
    ...
    # Cap transcript to most recent turns to avoid timeout
    MAX_INGEST_TURNS = 30
    if len(turns) > MAX_INGEST_TURNS:
        turns = turns[-MAX_INGEST_TURNS:]
    ...
    try:
        resp = cli_invoke(INGEST_PROMPT, transcript, timeout=120, model="opus")
        observations = resp["result"].get("observations", [])
    except Exception as e:
        write_learning_log("ingest_error", {"error": str(e)[:500], "transcript_len": len(transcript), "turn_count": len(turns)})
        observations = []
